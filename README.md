...
- **`--vgspv_csv_supervised_only`:** Train **only** on the Method-2 / VG-SPV train CSV: each step uses the row’s **`prompt`** and **`chosen_reasoning_trace`** (same contract as VG-fDPO), and **one epoch = one full pass over the CSV** (PaDT is not sampled for training). When **`--vgspv_eval_csv`** or the default **`test_method2.csv`** exists, **`eval_loss` is computed on that test CSV only** (PaDT / hub val eval is skipped). With default **``--save_every_steps 0``**, **eval runs each epoch** (aligned with HF checkpoint saves). For faster startup when PaDT is unused for training, pass **`--max_samples 1`** (or another small cap) on the HF train load so the hub dataset is tiny.
- **`--eval_early_stopping_patience`:** If **> 0** and eval is enabled, training stops after this many consecutive eval runs with no improvement in **`eval_loss`** (Hugging Face `EarlyStoppingCallback`). Also sets **`load_best_model_at_end`** on **`eval_loss`** and aligns **`save_steps`** with **`eval_steps`** so the best checkpoint can be restored. **`0`** (default) disables early stopping.
...
| Eval (HF + `eval_loss`) | **`--eval_split`**, **`--hf_eval_holdout_fraction`** (if no hub val), **`--eval_max_samples`**, **`--eval_steps`**, **`--eval_early_stopping_patience`**, **`--per_device_eval_batch_size`**, **`--skip_eval`** |
...
| Checkpoints | **`--save_every_steps`** (**`0`** = default: HF save + **`eval_loss`** at **end of each epoch**), **`> 0`** = step-based saves + step eval, **`--save_every_n_epochs`** (only with step saves; extra epoch saves), **`--save_total_limit`**, **`--eval_early_stopping_patience`** — each HF save also refreshes **`adapter_latest/`** |
